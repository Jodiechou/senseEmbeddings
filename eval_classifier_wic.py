...
if __name__ == '__main__':
	...
	with open(results_path, 'w') as results_f:  # store results in WiC's format
		for wic_idx, wic_entry in enumerate(load_wic(args.eval_set, wic_path='external/wic')):
			...
			ex1_sense_vec = ex1_sense_vec.cpu().detach()
			ex1_context_vec = torch.mm(W, ex1_curr_vector).squeeze(1).cpu().detach()
			...
			ex2_sense_vec = ex2_sense_vec.cpu().detach()
			ex2_context_vec = torch.mm(W, ex2_curr_vector).squeeze(1).cpu().detach()
			...
			# s1_sim = np.dot(ex1_curr_vector.cpu().detach(), ex2_curr_vector.cpu().detach())
