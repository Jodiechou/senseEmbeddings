...
def get_args(
	...
	# parser.add_argument('--sense_matrix_path', type=str, default='data/vectors/senseMatrix.semcor_diagonalI_{}_{}_5.txt'.format(emb_dim, emb_dim))
	parser.add_argument('--save_sense_matrix_path', default='data/vectors/senseMatrix.semcor_diagonal_{}_50.npz'.format(emb_dim))
	parser.add_argument('--save_weight_path', default='data/vectors/weight.semcor_diagonal_1024_{}_50.npz'.format(emb_dim))
			...
			for sent_info in batch:
				...
				for mw_idx, tok_idxs in idx_map_abs:
					...
					for sense in sent_info['senses'][mw_idx]:
						...
						##loss += (vec_c - A[index] * vec_g).norm() ** 2 + (A[index] - vector_ones).norm() ** 2  ### no weights with regulariser
