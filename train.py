...
def load_glove_embeddings():
	...
	embed_g = []
	with open('data/vectors/glove_embeddings.128.full.txt', 'r') as gfile:
		for line in gfile:
...
def load_bert_embeddings():
	...
	embed_c = []
	with open('data/vectors/bert_embeddings.128.full.txt', 'r') as cfile:
		for line in cfile:
			splitLine = line.split(' ')
	...
	for epoch in range (EPOCH):
		...
		for step, (train_g, train_c) in enumerate(train_loader):			
			for i in range (len(train_g)):
				wordEmbed = train_g[i]
				wordEmbed = np.reshape(wordEmbed, (1, 300))
				contEmbed = train_c[i]
				contEmbed = np.reshape(contEmbed, (1, 1024))
