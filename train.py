...
def get_args(
	...
	# parser.add_argument('-load_sv_path', help='Path to sense vectors', default='data/vectors/senseMatrix.semcor_diagonal_linear_large_bertlast4layers_multiword_{}_20.npz'.format(emb_dim))
...
if __name__ == '__main__':
	...
	if args.bert == 'large':
		...
		save_sense_matrix_path = 'data/vectors/senseMatrix.semcor_diagonal_relu_large_bertlast4layers_multiword_{}_50.npz'.format(args.emb_dim)
		save_weight_path = 'data/vectors/weight.semcor_diagonal_relu_bertlast4layers_multiword_1024_{}_50.npz'.format(args.emb_dim)
			...
			for sent_info in batch:
				...
				for mw_idx, tok_idxs in idx_map_abs:
					...
					for sense in sent_info['senses'][mw_idx]:		
						...
						loss += ((torch.mm(W[0], vec_c).squeeze(1)) - sense_vec).norm() ** 2
			...
			for valid_sent_info in valid_batch:
				...
				for valid_mw_idx, valid_tok_idxs in valid_idx_map_abs:
					...
					for valid_sense in valid_sent_info['senses'][valid_mw_idx]:
						...
						valid_loss += ((torch.mm(W[0], valid_vec_c).squeeze(1)) - valid_sense_vec).norm() ** 2
