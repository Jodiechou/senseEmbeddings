...
def get_args(
	...
	parser.add_argument('--sense_matrix_path', type=str, default='data/vectors/senses/senseMatrix.semcor_{}_{}.txt'.format(emb_dim, emb_dim))
...
if __name__ == '__main__':
	...
	for epoch in range(args.num_epochs):
		...
		for batch_num, i in enumerate(range(args.bsize, num_senses, args.bsize)):
			...
			if batch_num == num_batches: i = num_senses

			contEmbed = c_train[i-args.bsize:args.bsize].view(-1, 1024).to(device)
			g_train_vec = g_train[i-args.bsize:args.bsize].to(device)
			...
			embed = torch.matmul(g_train_vec, matrix_A_temp)
