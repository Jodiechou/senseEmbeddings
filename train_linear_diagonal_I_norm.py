...
def get_args(
	...
	parser.add_argument('--save_sense_matrix_path', default='data/vectors/senseMatrix.semcor_diagonalI_valid_norm_{}.npz'.format(emb_dim))
	parser.add_argument('--save_weight_path', default='data/vectors/weight.semcor_diagonalI_valid_norm_1024_{}.npz'.format(emb_dim))
			...
			for sent_info in batch:
				...
				for mw_idx, tok_idxs in idx_map_abs:
					...
					for sense in sent_info['senses'][mw_idx]:
						...
						vec_c = vec_c / vec_c.norm()

						loss += ((torch.mm(W[0], vec_c).squeeze(1)) - A[index] * vec_g).norm() ** 2 + (A[index] - vector_ones).norm() ** 2
						##loss += (vec_c - A[index] * vec_g).norm() ** 2 + (A[index] - vector_ones).norm() ** 2
			...
			for valid_sent_info in valid_batch:
				...
				for valid_mw_idx, valid_tok_idxs in valid_idx_map_abs:
					...
					for valid_sense in valid_sent_info['senses'][valid_mw_idx]:
						...
						valid_vec_g = valid_vec_g / valid_vec_g.norm()
						...
						valid_vec_c = valid_vec_c / valid_vec_c.norm()

						valid_loss += ((torch.mm(W[0], valid_vec_c).squeeze(1)) - A[index] * valid_vec_g).norm() ** 2 + (A[index] - vector_ones).norm() ** 2
