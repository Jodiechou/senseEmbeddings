...
def train(train_path, eval_path, glove_path, merge_strategy='mean', max_seq_len=512, max_instances=float('inf')):
	...
	logging.info('#Word embeddings: %d' % len(embed_c))

	logging.info('Writing Pre-trained GloVe Word Vectors ...')
	print("No of words in the vocabulary = %d" % len(embed_g))


    ## Get word embeddings only contain vectors
	outg_path = 'data/vectors/glove_embeddings.txt'
	outc_path = 'data/vectors/bert_embeddings.txt'
    ...
    ## Get the word embeddings format as word + vectors
