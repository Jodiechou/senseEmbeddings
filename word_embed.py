...
def getEmbeddings(train_path, eval_path, glove_path, merge_strategy='mean', max_seq_len=512, max_instances=float('inf')):
			...
			for sent_info, sent_bert in zip(batch, batch_bert):
				...
				for mw_idx, tok_idxs in idx_map_abs:
					...
					for sense in sent_info['senses'][mw_idx]:
						...
						# So we use a random vector 
						# for glove embedding for a word that does not exist in pre-trained glove embeddings						
						w = sense.split('%')[0]
						try:
							#if embed_g[sense]['vecs_num'] < max_instances:
							if w in glove_embed.keys():
								#print('found w', w)
								vec_g = glove_embed[w]
							...
							embed_g[sense] = {'vecs_sum': np.random.normal(scale=0.6, size=(300, )), 'vecs_num': 1}
	...
	outg_path = 'data/vectors/glove_embeddings.semcor.300.txt'
	outc_path = 'data/vectors/bert_embeddings.semcor.1024.txt'
	outs_path = 'data/vectors/senses.semcor.txt'
