...
def load_glove(glove_path):
	...
	for line in f:
		...
		gloveEmbed[word] = vec
	logging.info("Done. Loaded %d words" % len(gloveEmbed))
	return gloveEmbed


		
def getEmbeddings(train_path, eval_path, glove_path, merge_strategy='mean', max_seq_len=512, max_instances=float('inf')):
			...
			for sent_info, sent_bert in zip(batch, batch_bert):
				...
				for mw_idx, tok_idxs in idx_map_abs:
					...
					for sense in sent_info['senses'][mw_idx]:
						w = sense.split('%')[0]
						try:
							if embed_c[sense]['vecs_num'] < max_instances:

							   embed_c[sense]['vecs_sum'] += vec_c
							   embed_c[sense]['vecs_num'] += 1

						except KeyError:
							embed_c[sense] = {'vecs_sum': vec_c, 'vecs_num': 1}

						# Usually the numer of glove embeddings will be less than the numer of bert embeddings
						# This is because some words in the training corpus may not exist in the pre-trained glove embeddings
						# So we use a random vector for glove embedding for a word that does not exist in pre-trained glove embeddings
						try:
							w = sense.split('%')[0]
							if w in glove_embed:
								vec_g = glove_embed[w]
								embed_g[sense]['vecs_sum'] += vec_g
								embed_g[sense]['vecs_num'] += 1
							else:
								embed_g[sense]['vecs_sum'] += np.random.normal(scale=0.6, size=(300, ))

						except KeyError:
							embed_g[sense] = {'vecs_sum': vec_g, 'vecs_num': 1}
	...
	outg_path = 'data/vectors/glove_embeddings.small.txt'
	outc_path = 'data/vectors/bert_embeddings.small.txt'
	outs_path = 'data/vectors/senses.small.txt'
	with open(outg_path, 'w') as fvecs_out1:
		for sense_g, vecs_g in embed_g.items():
			vec_glove = vecs_g['vecs_sum'] / vecs_g['vecs_num']
			embed_str1 = ' '.join([str(v) for v in vec_glove.tolist()])
			fvecs_out1.write('%s %s\n' % (sense_g, embed_str1))
	logging.info('Written %s' % outg_path)


	with open(outc_path, 'w') as fvecs_out2:
		for sense_c, vecs_c in embed_c.items():
			vec_cont = vecs_c['vecs_sum'] / vecs_c['vecs_num']
			embed_str2 = ' '.join([str(v) for v in vec_cont.tolist()])
...
if __name__ == '__main__':
	...
	if args.dataset == 'semcor':
		train_path = args.wsd_fw_path + 'Training_Corpora/SemCor/small.data.xml'
		keys_path = args.wsd_fw_path + 'Training_Corpora/SemCor/small.gold.key.txt'
	...
	getEmbeddings(train_path, keys_path, args.glove_path, args.merge_strategy, args.max_seq_len, args.max_instances)